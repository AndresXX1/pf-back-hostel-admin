...
const getHourlyReservationStatistics = async (req, res) => {
    try {
        // Consulta para obtener estadísticas por hora
        const hourlyStats = await sequelize.query(
            `SELECT 
                TO_CHAR("createdAt" AT TIME ZONE 'America/Argentina/Buenos_Aires', 'YYYY-MM-DD') AS "date",
                TO_CHAR("createdAt" AT TIME ZONE 'America/Argentina/Buenos_Aires', 'HH24:00') AS "hour",
                COUNT(*) AS "reservationCount"
            FROM 
                "Reservas"
            WHERE 
                DATE("createdAt" AT TIME ZONE 'America/Argentina/Buenos_Aires') = CURRENT_DATE
            GROUP BY 
                "date", "hour"
            ORDER BY 
                "date" ASC,
        ...
        // Si hay estadísticas disponibles, devolverlas
        if (hourlyStats.length > 0) {
            return res.status(200).json(hourlyStats);
        }
        ...
        return res.status(404).json({ message: "No se encontraron estadísticas de reservas." });
...
module.exports = getHourlyReservationStatistics;