...
const getProductStatisticsMes = async (req, res) => {
    try {
        ...
        // Consulta para obtener las estadísticas de productos para el mes actual
        const monthStats = await getProductStatisticsForMonth(firstDayOfMonth, lastDayOfMonth);
        ...
        return res.status(404).json({ message: "No se encontraron estadísticas de productos para este mes." });
...
// Función para obtener las estadísticas de productos para un mes dado
const getProductStatisticsForMonth = async (startDate, endDate) => {
...
const formatResponse = (groupedStats, currentMonth) => {
    ...
    Object.keys(groupedStats).forEach(weekNumber => {
        ...
        const totalProductCount = weekStats.reduce((total, stat) => total + parseInt(stat.productCount), 0);
        response.push({ Week: `Semana ${weekNumber}`, productCount: totalProductCount.toString() });
    });
    return response;
};

module.exports = getProductStatisticsMes;