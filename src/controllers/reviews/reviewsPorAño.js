...
const getReviewStatisticsYear = async (req, res) => {
    try {
        // Obtener el año actual
        const currentYear = new Date().getFullYear();

        // Consulta para obtener las estadísticas de revisiones para el año actual
        const yearStats = await getReviewStatisticsForYear(currentYear);
        ...
        return res.status(404).json({ message: "No se encontraron estadísticas de revisiones para este año." });
...
// Función para obtener las estadísticas de revisiones para un año dado
const getReviewStatisticsForYear = async (year) => {
...
const formatResponse = (groupedStats, year) => {
    ...
    Object.keys(groupedStats).forEach(month => {
        ...
        const totalReviewCount = monthStats.reduce((total, stat) => total + parseInt(stat.reviewCount), 0);
        response.push({ Month: month, reviewCount: totalReviewCount.toString() });
    });
    return response;
};

module.exports = getReviewStatisticsYear;